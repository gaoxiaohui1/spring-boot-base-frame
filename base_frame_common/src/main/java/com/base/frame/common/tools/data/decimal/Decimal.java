package com.base.frame.common.tools.data.decimal;

import com.base.frame.common.tools.data.ToolClone;
...
public class Decimal extends Number implements Cloneable {
    ...
    private void init(Object initValue) {
        if (initValue instanceof Decimal) {
            ...
            throw new UnsupportedOperationException("初始化Decimal失败,传入值为" + ToolJson.modelToJson(initValue));
        }
    }
    ...
    public Decimal sqrt2(int scale) {
        ...
        if (scale > 13) {
            this.bigDecimal = com.base.frame.common.tools.data.decimal.ToolDecimal.sqrt(this.bigDecimal, scale, mathContext.getRoundingMode());
        } else {
    ...
    public String fullStrValue(int scale, RoundingMode roundingMode) {
        DecimalFormat decimalFormat = com.base.frame.common.tools.data.decimal.ToolDecimal.scale2Format(scale, roundingMode);
